...
public class BaseBoss1 : MonoBehaviour
{
    [Header("AI相关设置")]
    [Tooltip("是否需要入场动作")]
    ...
    protected const string EmbedBehaviourName = "Embed";

    private ActSeq entrance = new();
    private ActSeq hurt = new();
    ...
    public void SetHurt(ActSeq skillBody)
    {
        if (skillBody == null)
        {
            hurt.Clear();
            hasHurtSequence = false;
            return;
        }

        skillBody.CloneTo(ref hurt);
        hasHurtSequence = true;
    }
    ...
    {
        hurtSequencePlaying = false;
        hurtStateFinishing = false;
        Debug.Log($"[{enemyName}] Entering {HurtStateName}.");
        OnHurtStarted();

        if (hasHurtSequence)
        {
            hurt.Play(this);
            hurtSequencePlaying = true;
        }
    }

    private void OnHurtStay()
    {
        if (hurtSequencePlaying)
        {
            if (!hurt.IsPlaying)
            {
                hurtSequencePlaying = false;
    ...
    {
        var requestedBehaviour = !string.IsNullOrWhiteSpace(forcedNextBehaviourName)
            ? forcedNextBehaviourName
            : DecideNextBehaviour();
        forcedNextBehaviourName = string.Empty;
    ...
    protected void ForceNextBehaviour(string behaviourName)
    {
        ...
        forcedNextBehaviourName = behaviourName;
    }

    protected void RequestEmbedAfterHurt()
    {
        shouldEmbedAfterHurt = true;
    }

    private void TryEnterHurtState(HitInfo incoming)
    {
        if (!ShouldEnterHurtState(incoming))
        {
            return;
        }
        ...
        if (enemyStateMachine.CurrentStateName == HurtStateName ||
            enemyStateMachine.CurrentStateName == DeathStateName)
        {
            return;
        }

        enemyStateMachine.TransitionTo(HurtStateName);
        Debug.Log($"[{enemyName}] Transitioning to {HurtStateName}.");
    }

    protected virtual bool ShouldEnterHurtState(HitInfo incoming)
    {
        return false;
    }

    protected virtual void OnHurtStarted()
    {
        // derived types can hook
    }

    protected virtual void OnHurtEnded()
    {
        // derived types can hook (e.g. to request embed)
    }

    private void FinishHurtState()
    {
        if (hurtStateFinishing || enemyStateMachine == null)
        {
            return;
        }

        hurtStateFinishing = true;
        OnHurtEnded();

        if (shouldEmbedAfterHurt)
        {
            ForceNextBehaviour(EmbedBehaviourName);
            shouldEmbedAfterHurt = false;
        }

        enemyStateMachine.TransitionTo(DecisionStateName);
        Debug.Log($"[{enemyName}] Hurt finished. Transitioning to {DecisionStateName}.");
        hurtStateFinishing = false;
    }
